...
# === Configuration ===
CSV_FILE = "/scratch/polsm/011-DRL-experimental/AFC-DRL-experiment-v3/09-test-CTA-DRL/logs/PPO_V1noUDP_20250703-114231/live_rewards.csv"
...
while True:
    try:
        ...
        if df is not None and not df.empty:
            ...
            axs[0].plot(df["step"], df["reward"], label="Reward", linewidth=1.5, color='black', marker='o', markersize=2, alpha=0.05)
            ...
            axs[0].grid(True)
            axs[0].legend()
            axs[1].plot(df["step"], df["action"], 'o', label="action vs step", markersize=2, alpha=0.05, color='blue')
            axs[1].set_xlabel("step")
            axs[1].set_ylabel("action")
            axs[1].set_title("action vs step")
            axs[1].grid(True)
            axs[1].legend()
            axs[2].plot(df["obs2"], df["obs3"], 'o', label="obs3 vs obs2", markersize=2, alpha=0.05, color='red')
            axs[2].set_xlabel("obs[2]")
            axs[2].set_ylabel("obs[3]")
            axs[2].set_title("obs[3] vs obs[2]")
            axs[2].grid(True)
            axs[2].legend()
            axs[3].plot(df["obs2"], df["action"], 'o', label="action vs obs2", markersize=2, alpha=0.005, color='green')
            axs[3].set_xlabel("obs[2]")
            axs[3].set_ylabel("action")
            axs[3].set_title("action vs obs[2]")
            axs[3].set_ylim(-3, 3)
            axs[3].set_xlim(-2, 2)
            axs[3].grid(True)
            axs[3].legend()
